    ...
    if (candidate.length > 0) {
        ...
        for (let i in candidate.photo) {
            if(candidate.photo[i].small){
                photoHTML += `<div class="division-right-image small"><img src="images/${candidate.photo[i].url}" alt="">${candidate.photo[i].subtitle}</div>`;
            }else{
...
function confirma() {
    ...
    if(voteConfirmed) { 
        ...
        if(stages[currentStage] !== undefined) {
            ...
            document.querySelector('.screen').innerHTML = '<div class="notification--exlarge blink">FIM</div>';
        }
    }
